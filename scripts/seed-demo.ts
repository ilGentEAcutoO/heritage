...
function buildSeedSql(): string {
  const lines: string[] = [
    ...
    '',
    '-- Trees',
    `INSERT OR IGNORE INTO trees (id, slug, name, name_en, owner_id, visibility) VALUES` +
    ` (${esc(TREE_ID)}, ${esc(TREE_SLUG)}, ${esc('บ้านวงศ์สุริยา')}, ${esc('Wongsuriya Family')}, NULL, 'public');`,
    '',
    '-- People',
  ];
